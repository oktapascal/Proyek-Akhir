...
<div class="block-header">
    ...
    <div class="row clearfix">
             ...
             <div class="card">
                  ...
                  <h2>TAMBAH BAHAN UTAMA<small>Sebelum Simpan, Pastikan Anda Sudah Yakin Untuk Menyimpan Data Bahan Utama ini.</small></h2>
                </div>
                <div class="body">
              <?php echo form_open('BahanUtama_Controller/SimpanBahanUtama', array('id'=>'TambahBahanUtama')); ?>
			    			...
			    			<button type="submit" class="btn btn-block btn-lg btn-primary waves-effect">SIMPAN BAHAN UTAMA</button>
						<?php echo form_close(); ?>
                        </div>
                    </div>
       		    </div>
     ...
     <script type="text/javascript">
     	$(document).ready(function(){
            ...
            $(element).parents('.form-group').append(error);
             ...
             $('#TambahBahanUtama').submit(function(e){
                e.preventDefault();
                $.ajax({
                    url: $('#TambahBahanUtama').attr('action'),
                    type: $('#TambahBahanUtama').attr('method'),
                    data: $('#TambahBahanUtama').serialize(),
                    dataType: 'json',
                    success: function(respon){
                        $('input[name=csrf_test_name]').val(respon.token);
                        if(respon.success == true)
                        {
                            $('#TambahBahanUtama')[0].reset();
                            ...
                            var text      = "Data Bahan Utama Berhasil Dimasukkan";
                            showNotification(colorName, text, placementFrom, placementAlign, animateEnter, animateExit);

                            Push.Permission.request(() => {
                            Push.create('NOTIFIKASI', {
                            body: 'Data Bahan Utama Berhasil Dimasukkan',
