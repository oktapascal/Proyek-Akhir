    	...
    	<div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
    		<div class="card">
    			<div class="header bg-blue-grey">
    				<h2>Silahkan Pilih Tanggal Kehadiran (Presensi)</h2>
    			</div>
    			<div class="body">
    			<?php echo form_open('Presensi_Controller/PilihPresensi', array('id'=>'PilihPresensi')); ?>
					...
					<div class="form-line">
					<input type="text" name="tanggal_presensi" id="tanggal_presensi" class="datepicker form-control" autocomplete="off" placeholder="Tanggal Presensi...">
					</div>
					</div>
    ...
    <script type="text/javascript">
    	...
    	$('#PilihPresensi').validate({
    	  ...
    	  //Submit Pilih Tanggal//
             $('#PilihPresensi').submit(function(e){
                e.preventDefault();
                $.ajax({
                    url: $('#PilihPresensi').attr('action'),
                    type: $('#PilihPresensi').attr('method'),
                    data: $('#PilihPresensi').serialize(),
                    dataType: 'json',
                    success: function(respon){
                        $('input[name=csrf_test_name]').val(respon.token);
                        if(respon.success == true)
                        {
                            window.location.replace('<?php echo site_url('Presensi_Controller/index'); ?>')
                        }
                        else{
