<?php 
class BahanUtama_Model extends CI_Model
{
	...
	private function getBahanUtama()
	{
		...
		foreach($this->search as $data)
		{
			if($_GET['search']['value']) //Jika datatable mengirimkan pencarian dengan metode GET//
			{
				...
				if(count($this->search) - 1 == $bahanutama)
				{
					$this->db->group_end();
				}	
			}
	...
	public function getDataTableBahanUtama()
	{
		$this->getBahanUtama();
		if($_GET['length'] != -1)
		{
	...
	public function KodeBahanUtama()
    {    
    	...
    	//jika kode belum ada      
    		...
    		$kodejadi = "BHU-".$kodemax;     
    	return $kodejadi;  
    }

	public function UpdateBahanUtama($kode, $nama, $satuan)
	{
		...
		$notifikasi = "Kode Bahan Utama ".$kode." Diubah";
		$icon       = "local_activity";
	...
	public function InsertBahanUtama($nama, $satuan)
	{
		$kode = $this->KodeBahanUtama();
		$bahan = "INSERT INTO `bahan`(`id_bahan`, `nama_bahan`, `stok_digudang`, `stok_diproduksi`, `satuan`, `jenis_bahan`) VALUES (?,?,?,?,?,?)";
		$notif = "INSERT INTO `notifikasi`(`notifikasi`, `icon`, `waktu`, `status`, `id_posisi`) VALUES (?,?,?,?,?)";
		$notifikasi = "Kode Bahan Utama ".$kode." Dimasukkan";
		$icon       = "local_activity";
		date_default_timezone_set("Asia/Jakarta");
		$waktu      = date("H:i:s");
		$status     = 0;
		$this->db->trans_begin();
		$this->db->query($bahan, array($kode, $nama, 0, 0, $satuan, "Bahan Utama"));
		//Insert Notifikasi//
		$posisi = $this->TarifPosisi_Model->GetPosisi();
		foreach($posisi as $ps)
		{
